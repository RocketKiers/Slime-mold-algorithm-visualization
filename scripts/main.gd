...
var map_width: int
var map_height: int
var trail_map: Array # A 2d map array to hold pheremone values
...
# Variable for the number of agents
@export var number_of_agents: int = 1000
@export_range(10.0, 500.0) var AGENT_SPEED: float = 250.0 # speed of the moving agents
@export var var_agent_size: float = 2.0 #changable agent size (appended to a abs(sin) wave
...
# array om alle agents/objecten te houden
...
# --- NEW: Array to hold spawn positions (Vector2)
...
func _ready():
	...
	# Initialize the 2D array (every coordinate filled with 0.0)
...
func _initialize_agents():
	...
	# Get the area where agents can start (e.g., the screen size)
	var screen_size = get_viewport_rect().size
	...
	for i in number_of_agents:
		...
		var middle_y = screen_size.y / 2
		var fixed_direction = i * 360.0 / number_of_agents  # angle in degrees
		var radius = 100
		var angle = deg_to_rad(fixed_direction)  # convert degrees to radians
		var start_x = middle_x + cos(angle) * radius
		var start_y = middle_y + sin(angle) * radius
		
		var start_position = Vector2(start_x, start_y)
		
		# 2. Initial Direction: Random angle in radians (from 0 to 2*PI, or TAU)
...
func _deposit_pheromone(agent: Agent):
	...
	var x = int(map_coords.x)
	var y = int(map_coords.y)
	
	# Deposit the pheromone (clamping it to a max value, e.g., 10.0, to prevent overflow)
	trail_map[x][y] = min(trail_map[x][y] + DEPOSIT_AMOUNT, 10.0)


# _process wordt elke frame gecalled, delta is tijd sinds laatste call
func _process(delta):
	time += delta
	agent_size = (abs(sin(time) * var_agent_size)+3)
	queue_redraw()
	var sub_delta = delta / SIMULATION_STEPS_PER_FRAME

	for i in SIMULATION_STEPS_PER_FRAME:
		_move_agents(sub_delta)
...
func _move_agents(delta: float):
	...
	for agent in agents:
		...
		# The old random turn is removed, as it's now integrated into the steering logic:
		# agent.direction += randf_range(-0.5, 0.5)
